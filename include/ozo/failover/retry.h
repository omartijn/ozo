...
constexpr auto make_errcs_tuple(const ErrorConditions ...errcs) {
...
public:
     ...
     *                  see `ozo::retry_strategy::times()` for details about calculations.
     */
     ...
     */
    ErrorConditions retry_conditions() const { return errcs_; }
    ...
    std::optional<basic_try> get_next_try(error_code ec, Connection&& conn) const {
        ...
        if (tries_remain()) {
            auto retval = basic_try(tries_remain() - 1, retry_conditions(), ctx_);
...
private:
    ...
    bool can_retry([[maybe_unused]] error_code ec) const {
        ...
        if constexpr (decltype(hana::is_empty(retry_conditions()))::value) {
            return true;
        } else {
            const auto f = [ec](bool v, auto errc){ return v || (ec == errc); };
            return boost::hana::fold(retry_conditions(), false, f);
        }
    }
...
public:
    retry_strategy(ErrorConditions errcs) : errcs_(std::move(errcs)) {}
    ...
    auto get_first_try(const Operation&, const Allocator& alloc,
        ...
        return basic_try(get_times(), retry_conditions(), std::move(ctx));
    }
     ...
     * @endcode
     */
    auto times(int n) const {
        if (n < 0) {
            throw std::invalid_argument(
                "ozo::failover::retry_strategy::times() argument "
    ...
    /**
     * @brief Sintactic sugar for `ozo::retry_strategy::times()`
     *
     * @param n --- number of tries
     * @return `retry_strategy` specialization object
     */
    auto operator * (int n) const { return times(n); }

    /**
     * @brief number of operation tries setted with `ozo::retry_strategy::times()`
     *
 ...
 * See `ozo::failover::retry_strategy::times()` for time constraint details.
 *
 * @code
